...
import {
  db, users, submissions, pushSubscriptions,
} from '@lib/db';
  ...
  if (isBuilderModel(contentType)) {
    ...
    switch (contentType) {
      ...
      case 'notificationHistory': {
        // These are read-only system logs and cannot be imported
        errors.push(`${contentType} cannot be imported - this is a system-generated log`);
        break;
      }
    }
  }
...
async function performImport(
  ...
  if (isBuilderModel(contentType)) {
    ...
    switch (contentType) {
      ...
      case 'notificationHistory': {
        // These are read-only system logs and cannot be imported
        // This case should never be reached due to validation, but handle it for exhaustiveness
        break;
      }
    }
  }
