...
export function isValidExportData(data: unknown): data is ExportData<unknown> {
	...
	if (typeof metadata.exportDate !== 'string') return false;
	if (typeof metadata.contentType !== 'string') return false;
	if (typeof metadata.itemCount !== 'number') return false;
	if (metadata.source !== 'builder' && metadata.source !== 'database') return false;

	// Check items array
	if (!Array.isArray(exportData.items)) return false;
