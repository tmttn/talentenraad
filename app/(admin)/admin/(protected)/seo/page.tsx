        ...
        <div className='flex gap-1 mb-3'>
          <div
            className={`flex-1 h-1.5 rounded-full ${
              analysis.fieldScores.title.status === 'good'
                ? 'bg-green-500'
          ...
          <div
            className={`flex-1 h-1.5 rounded-full ${
              analysis.fieldScores.description.status === 'good'
                ? 'bg-green-500'
...
function TopIssuesPanel({items}: Readonly<{items: ContentItem[]}>) {
  const topIssues = useMemo(() => {
    ...
    if (missingTitles > 0) {
      issues.push({
        type: 'missing_title', count: missingTitles, impact: 25, label: 'Ontbrekende titels', icon: <FileText className='w-4 h-4' />,
      });
    }

    if (missingDescriptions > 0) {
      issues.push({
        type: 'missing_description', count: missingDescriptions, impact: 25, label: 'Ontbrekende samenvattingen', icon: <FileText className='w-4 h-4' />,
      });
    }

    if (missingImages > 0) {
      issues.push({
        type: 'missing_image', count: missingImages, impact: 15, label: 'Ontbrekende afbeeldingen', icon: <ImageIcon className='w-4 h-4' />,
      });
    }

    if (shortTitles > 0) {
      issues.push({
        type: 'short_title', count: shortTitles, impact: 10, label: 'Te korte titels', icon: <FileText className='w-4 h-4' />,
      });
    }

    if (shortDescriptions > 0) {
      issues.push({
        type: 'short_description', count: shortDescriptions, impact: 10, label: 'Te korte samenvattingen', icon: <FileText className='w-4 h-4' />,
      });
    }
...
export default function SeoDashboardPage() {
  ...
  const analyzedItems = useMemo(
    () =>
      allItems.map(item => ({
        item,
        analysis: analyzeSeo({
          title: item.title, description: item.description, image: item.image, content: item.content,
        }),
      })),
    [allItems],
  );
  ...
  const stats = useMemo(() => {
    ...
    return {
      avgScore, perfectCount, needsWork, criticalCount, newsAvg, activitiesAvg, totalQuickWinPoints, potentialScore,
    };
  }, [analyzedItems]);
