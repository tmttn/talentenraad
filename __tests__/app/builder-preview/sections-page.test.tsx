...
describe('SectionPreviewPage', () => {
  ...
  it('refetches when model changes', async () => {
    ...
    await waitFor(() => {
      expect(mockFetchOneEntry).toHaveBeenCalledWith(expect.objectContaining({model: 'model-1'}));
    });
    ...
    await waitFor(() => {
      expect(mockFetchOneEntry).toHaveBeenCalledWith(expect.objectContaining({model: 'model-2'}));
    });
  });
});
